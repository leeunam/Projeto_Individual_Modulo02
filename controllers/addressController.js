...
const createAddress = async (req, res) => {
  try {
    ...
    const newAddress = await addressModel.create({street, number, district, cep});
...
const updateAddress = async (req, res) => {
  try {
    const { street, number, district, cep } = req.body;
    const updatedAddress = await addressModel.update(req.params.id, {street, number, district, cep});
    if (updatedAddress) {
